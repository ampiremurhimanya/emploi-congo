...
router.get('/public/search', async (req, res) => {
  try {
    ...
    if (keyword) {
      conditions.push(`(LOWER(o.title) LIKE $${i} OR LOWER(o.company_name) LIKE $${i} OR LOWER(o.description) LIKE $${i})`);
    ...
    const countResult = await pool.query(
      `SELECT COUNT(*) FROM opportunities o WHERE ${where}`, params
    );
...
router.get('/public/:id', async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT * FROM opportunities WHERE id = $1', [req.params.id]
    );
...
router.get('/stats', async (req, res) => {
  try {
    const opps = await pool.query(`SELECT COUNT(*) FROM opportunities WHERE status = 'APPROVED'`);
